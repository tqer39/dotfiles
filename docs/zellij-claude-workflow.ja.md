...
│       ├── Zellij tab 1: main      ← yazi + Claude Code + shell
│       │   ├── pane: files  (20%)  ← yazi
│       │   ├── pane: claude (55%)
│       │   └── pane: shell  (25%)
...
`claude-code` レイアウト左端で常時起動している。dotfiles が配る独自キーは 2 つ。
...
- `src/.config/zellij/layouts/claude-code.kdl` — Claude Code レイアウト（3 ペイン）
