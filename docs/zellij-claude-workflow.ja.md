...
│       ├── Zellij tab 1: main      ← yazi + shell
│       │   ├── pane: files (20%)   ← yazi
│       │   └── pane: shell (80%)   ← claude / 通常シェル共用
...
- 左端の `yazi` ペインは常時ファイルエクスプローラとして待機し、選択したディレクトリへ隣のシェルペインを `cd` させたり、git ルート基準の相対パスをコピーしたりできる
...
- `~/.config/yazi/yazi.toml` → `src/.config/yazi/yazi.toml`
- `~/.config/yazi/keymap.toml` → `src/.config/yazi/keymap.toml`
- `~/.config/yazi/scripts/cd-shell-pane.sh` → `src/.config/yazi/scripts/cd-shell-pane.sh`
- `~/.config/yazi/scripts/yank-relative.sh` → `src/.config/yazi/scripts/yank-relative.sh`

`yazi` も Homebrew で別途インストール:
...
3. **Zellij セッション起動（default レイアウト）**
   ...
   - 初回: 指定名のセッションを新規作成、`default` レイアウトで左に yazi・右側がシェル
...
### Yazi（左ペインのファイルエクスプローラ）

`default` レイアウト左端で常時起動している。dotfiles が配る独自キーは 2 つ。

| キー | 動作 |
| --- | --- |
| `c d` | ホバー中の項目（ファイルなら親ディレクトリ）へ、右隣のシェルペインを `cd` |
| `y r` | ホバー中のパスを git ルート基準の相対パスにしてクリップボードへコピー（非 git 配下では絶対パスにフォールバック） |

その他の移動・選択・プレビューは yazi デフォルトのまま使える（`j`/`k`/`h`/`l`、`<Enter>`、`q` 終了など）。

`c d` の裏側では `zellij action move-focus right` → `write-chars "cd <path>"` → `write 13` を叩いている。右隣のペインが **シェルでプロンプト入力待ち** の状態であることが前提で、Claude Code ペインにフォーカスが向いている場合は意図しないタイピングになるので注意。
...
- `src/.config/zellij/layouts/default.kdl` — default レイアウト（yazi + shell の 2 ペイン）
- `src/.config/yazi/yazi.toml` — yazi 本体設定
- `src/.config/yazi/keymap.toml` — yazi カスタムキー
- `src/.config/yazi/scripts/cd-shell-pane.sh` — シェルペインへの cd ヘルパ
- `src/.config/yazi/scripts/yank-relative.sh` — git ルート相対パスコピーヘルパ
