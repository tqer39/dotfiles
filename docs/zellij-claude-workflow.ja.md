...
### R4. ファイルパスをターミナルに貼り付ける

1. Claude ペインや shell ペインにフォーカスを置いた状態で `Alt+e`
2. フローティングで strider（ファイラー）が開く
3. `h/j/k/l` または矢印キーでディレクトリ移動、`Enter` でファイル選択
4. 選択したファイルのパスが元のペインに自動挿入される

Claude Code にファイルを指定したいとき便利。
