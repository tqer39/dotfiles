...
**Goal:** `mise.toml` を tool version の SoT に集約し、`.python-version` / `.terraform-version` を削除、GitHub Actions terraform workflow を `jdx/mise-action` 経由に移行する。

**Architecture:** ルート `mise.toml` の `[tools]` テーブルに python を追記して 4 ツール（python / terraform / trivy / biome）の唯一の宣言場所とする。CI も同 SoT を使うよう terraform workflow に `jdx/mise-action@v4.0.1` を追加し、`hashicorp/setup-terraform` と `.terraform-version` を `cat` する step、および dead env (`TF_VERSION`, `AWS_ACCOUNT_ID` export) を撤去する。anyenv 廃止は別 spec (`ref-anyenv-removal`) に切り出し、本 plan の scope 外。
...
Expected: `Installing pyenv...` までは出るが、`.python-version` 不在のため `pyenv install` の log は出ない（`if [[ -f "$python_version_file" ]]` で skip）。
...
両方とも環境変数を定義/出力するだけで、後続 step がどこも `${{ env.TF_VERSION }}` `${{ env.AWS_ACCOUNT_ID }}` を読まないことを目視確認する。
...
`Checkout` step（`uses: actions/checkout@de0fac2e4500dabe0009e67214ff5f5447ce83dd # v6`）の **直後**、`Configure AWS Credentials` の前に以下を挿入:
...
Run: `just lint`
Expected: 全 hook PASS。prettier が YAML を formatting check し、PASS することを確認。
...
- 該当が **0 件** なら Step 4.2 へ skip。
- 該当があれば各ファイルを読んで、内容に応じて以下のいずれかを行う:
  - 「`mise.toml` で管理」と書き換え
  - 単に削除（古い手順の残骸の場合）
...
- 該当が **0 件** なら Step 4.3 へ skip。
- 該当があり、リポ全体（`rg -iw '<word>' --hidden`）でも参照されない場合は削除する。
...
PR ページで GitHub Actions terraform workflow が green になることを確認。失敗した場合は失敗 step の log を確認し、必要なら本 plan に修正タスクを追加して対応。
